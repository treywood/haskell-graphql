...
data Interface a = Interface String [Field a]
    ...
    show (ObjectType name interfaces fields) =
        let
            implements =
                 ...
                 else " implements " ++ (intercalate " & " (map (\(Interface name _) -> name) interfaces))
        in
            "type " ++ name ++ implements ++ " {\n  " ++
            (intercalate "\n  " $ map (\(Field name fieldType _) -> name ++ ": " ++ (typeName fieldType)) fields) ++
            "\n}"
...
typeName (NullableType innerType) =
    let
        rawName = typeName innerType
    in
        take (length rawName - 1) rawName
...
printSchema schema = (intercalate "\n\n" allTypeStrings) ++ "\n"
    where
        allTypeStrings = (reverse . unique) $ map show (findTypes (TypeBox schema) [] [])
        findTypes schema types typeNames =
            case schema of
                TypeBox (ObjectType name _ fields) ->
                    if (elem name typeNames) then
                        types
                    else
                        schema:(concat $ map
